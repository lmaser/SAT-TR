param(
    [int] $Iterations = 1,
    [int] $MaxCandidates = 24,
    [string] $Profile = 'fast',
    [double] $StaticWorsening = 0.015,
    [double] $HammerRelativeImprovement = 0.002,
    [double] $HammerAbsoluteImprovement = 0.05,
    [int] $FoundationExactLimit = 8,
    [string] $ResidualCandidates = 'replace:0.25,replace:0.5,replace:0.75'
)

$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
Set-Location -LiteralPath $root

python SAT-TR\tools\iterate_overdrive_ts808.py `
    --iterations $Iterations `
    --strict-missing `
    --pre-cascade-fit off `
    --post-local-fit off `
    --candidate-residuals $ResidualCandidates `
    --core-mode off `
    --core-hammer-fit verified `
    --core-hammer-profile $Profile `
    --core-hammer-max-candidates $MaxCandidates `
    --core-hammer-static-worsening $StaticWorsening `
    --core-hammer-relative-improvement $HammerRelativeImprovement `
    --core-hammer-absolute-improvement $HammerAbsoluteImprovement `
    --control-fit off `
    --jobs 1 `
    --foundation-exact-limit $FoundationExactLimit
$iterationExitCode = $LASTEXITCODE

if ($iterationExitCode -eq 0) {
    $exportScript = 'E:\Workspace\Production\JUCE_projects\SAT-TR\tools\export_overdrive_voicing_header.ps1'
    if (Test-Path -LiteralPath $exportScript -PathType Leaf) {
        powershell -ExecutionPolicy Bypass -File $exportScript
    }
    else {
        Write-Warning "Overdrive voicing export script not found, export skipped: $exportScript"
    }
}
else {
    Write-Warning "TS808 core-Hammer refine failed with code $iterationExitCode; overdrive voicing export skipped."
}

exit $iterationExitCode
